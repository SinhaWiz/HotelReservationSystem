...
CREATE TABLE vip_members (
  ...
  membership_level    VARCHAR2(20) NOT NULL
                      CHECK (membership_level IN ('GOLD','PLATINUM','DIAMOND')),
...
CREATE TABLE bookings (
  ...
  booking_status    VARCHAR2(20) DEFAULT 'CONFIRMED'
                    CHECK (booking_status IN ('CONFIRMED','CHECKED_IN','CHECKED_OUT','CANCELLED','NO_SHOW')),
...
CREATE TABLE invoice_line_items (
  ...
  item_type        VARCHAR2(20)
                   CHECK (item_type IN ('ROOM','SERVICE','TAX','DISCOUNT','EXTRA_CHARGE')),
